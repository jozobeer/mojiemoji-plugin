...
from mojiemoji.prestamp.emoji_pass import _emoji_transform_line
from mojiemoji.prestamp.incremental import processed_sentences_masked
from mojiemoji.prestamp.lines import _observe_summary_tags, _walk_lines_outside_fences_with_reason
from mojiemoji.prestamp.masker import _Masker
from mojiemoji.prestamp.text_pass import _transform_line
from mojiemoji.prestamp.unstamped_report import report_unstamped
...
# `from mojiemoji.prestamp import prestamp_text`" wording. transform() is the
...
def main(
    ...
    *,
    suppress_markdown: bool = False,
) -> int:
    """Run the CLI over stdin.

    ``suppress_markdown`` echoes the untouched input instead of the
    transformed markdown, while still writing any requested unstamped
    report. It is an output control, not a policy: deciding *when* to
    suppress belongs to the caller, so that no host-specific rule (a
    GitHub surface, a repository setting) reaches this package.
    """
    parser = argparse.ArgumentParser(
        description="Replace catalog terms in markdown with mojiemoji stamps.",
        usage=(
            "mojiemoji [--seed SEED] [--base-url URL] "
            "[--catalog PATH] < input.md > output.md"
        ),
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Base URL for the mojiemoji service. Falls back to "
            "$MOJIEMOJI_BASE_URL, then the hosted default."
        ),
    )
    parser.add_argument(
        "--catalog",
    ...
    from mojiemoji.lib.config import get_intensity as _get_config_intensity
    ...
    # the body's Japanese and is independent of any caller policy, so
    # suppression must not silence it. Only markdown stdout respects it.
    output = transform(
