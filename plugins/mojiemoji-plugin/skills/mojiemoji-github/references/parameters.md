...
| Katakana | **3** | |
| ASCII | **3** (JP) / **~8** (English) | 短い略語（WIP, API, LGTM）は3字まで。英語単語（DONE, MERGE, REVIEW, BUILD...）は i18n 対応で上限を緩和 (#148) |
...
python3 "<MOJIEMOJI_GITHUB_SKILL_DIR>/scripts/mojiemoji_markdown.py" --text '修正' --inline ...

# 2 スタンプ分割: チャンクを別々にレンダリングして inline で連結
# (区切り無し):
python3 "<MOJIEMOJI_GITHUB_SKILL_DIR>/scripts/mojiemoji_markdown.py" --text 'マージ' --inline ...
python3 "<MOJIEMOJI_GITHUB_SKILL_DIR>/scripts/mojiemoji_markdown.py" --text '歓迎'   --inline ...
