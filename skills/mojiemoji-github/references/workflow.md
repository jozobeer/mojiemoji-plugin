...
`<MOJIEMOJI_GITHUB_SKILL_DIR>` means the absolute directory containing the
active `mojiemoji-github/SKILL.md`. Resolve it from the loaded skill path; do
not derive it from the shell working directory or harness-specific environment
variables.

The selector subagent and PreToolUse hook are optional harness capabilities.
Use them only when available. The Codex package ships neither, so use the
direct-script path with varied parameters and perform the documented manual
...
[3] AI が catalog 外の語に追加装飾  <-- selector or direct-script fallback
      ↓ final body
[4] PreToolUse hook (when available) or manual verification
      ↓
[5] 投稿 (gh / MCP github_*)
```
...
```bash
python3 "<MOJIEMOJI_GITHUB_SKILL_DIR>/scripts/prestamp.py" \
  < body.md > body-pre.md
...
```bash
python3 "<MOJIEMOJI_GITHUB_SKILL_DIR>/scripts/prestamp.py" \
  < draft.md | gh pr create --body-file -
```
...
5. パラメータを選ぶ:
   ...
   - **Capability override:** If `mojiemoji-selector` is unavailable, do not invent or dispatch it. Read the packaged flavor guide and presets, choose diverse parameters directly, and continue with manual verification. This overrides the mandatory-selector wording in this workflow.
...
9. 本文が固まったら `<MOJIEMOJI_GITHUB_SKILL_DIR>/scripts/coverage.py --surface <...>` で最低密度/文ヒット率/段落偏りを測る。<img src="https://mojiemoji.jozo.beer/emoji/%E8%AD%A6%E5%91%8A?font=gothic&amp;color=fb923c&amp;animation=disco&amp;background=transparent&amp;outline_width=0" alt="警告" height="24" align="absmiddle">が出たら装飾を足し、hook 経路で block したい時は `--mode block` を使う。
...
SKILL_DIR: <MOJIEMOJI_GITHUB_SKILL_DIR>
```
...
```bash
# Block
python3 "<MOJIEMOJI_GITHUB_SKILL_DIR>/scripts/mojiemoji_markdown.py" --text 'レビュー歓迎' \
  --font maru-bold --color 3b82f6 --animation bane --speed slow

# Inline (height=24 align=absmiddle)
python3 "<MOJIEMOJI_GITHUB_SKILL_DIR>/scripts/mojiemoji_markdown.py" --text 'マジで' --inline \
