"""Tests for mojiemoji.lib.term_boundaries.
...
Coverage:
  ...
  4. `prestamp.boundaries` re-exports the same character classes, so
...
def test_is_ascii_key_classification(term: str, expected: bool) -> None:
    from mojiemoji.lib.term_boundaries import is_ascii_key
...
def test_prestamp_and_helper_agree_on_ascii_keys() -> None:
    ...
    prestamp = importlib.import_module("mojiemoji.prestamp")
    from mojiemoji.lib.term_boundaries import is_ascii_key
...
def test_bounded_re_matches_standalone_only() -> None:
    from mojiemoji.lib.term_boundaries import bounded_re
...
def test_helper_constants_match_prestamp_constants() -> None:
    ...
    prestamp = importlib.import_module("mojiemoji.prestamp")
    from mojiemoji.lib import term_boundaries
...
def test_han_range_excludes_hangul_yi_pua(char: str, expected: bool) -> None:
    ...
    Markdown skill.
    """
    from mojiemoji.prestamp.boundaries import HAN_CHAR_RE
